...
//    루트로 들어와도 걸리고 아무것도 안 적어도 걸리고 , 스프링버전이 5 미만이면 @RequestMapping 사용을 이렇게
    ...
    @GetMapping("/me")
    public String showMe(@RequestParam("resv_no")Long id,
                         Model model){
        VaccresvDTO vaccresvDTO = mainService.findOne(id);
        if (vaccresvDTO != null) {
            ...
            // 날짜 포맷 설정
//            String formattedDate = vaccresvDTO.getResv_date().format(DateTimeFormatter.ofPattern("yyyy년 MM월 dd일"));
//            vaccresvDTO.setResv_date(formattedDate);
